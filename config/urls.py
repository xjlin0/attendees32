...
urlpatterns = [
    ...
    path(
        "",
        kwargs={"repo_version": f'{"DETACHED_" if repo.head.is_detached else repo.active_branch.name}/{repo.git.rev_parse(repo.head, short=True) if repo.head else "no sha"}'},
